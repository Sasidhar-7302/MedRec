"""Comprehensive test suite for MedRec application with Windows compatibility."""
...
class MedRecTester:
    ...
    def run_all_tests(self) -> bool:
        ...
        tests = [
            ...
            ("Accuracy Analysis", self.test_accuracy),
        ]
    ...
    def test_environment(self) -> bool:
        ...
        try:
            ...
            print("   [WARN] faster-whisper not installed (will use whisper.cpp)")
        
        print("\n3. Checking Whisper binary...")
        whisper_binary = Path(self.config.whisper.binary_path)
        if whisper_binary.exists():
            print(f"   [OK] Whisper binary found: {whisper_binary}")
        else:
            print(f"   [FAIL] Whisper binary missing: {whisper_binary}")
            all_ok = False
        ...
        if whisper_model.exists():
            ...
            print(f"   [OK] Whisper model found: {whisper_model} ({size_mb:.1f} MB)")
        else:
            print(f"   [FAIL] Whisper model missing: {whisper_model}")
            all_ok = False
    ...
    def test_audio_system(self) -> bool:
        ...
        print("\n2. Testing audio recording (3 seconds)...")
        try:
            ...
            if test_audio.exists() and test_audio.stat().st_size > 0:
                ...
                print(f"   [OK] Recording successful ({size_kb:.1f} KB)")
                # Keep file for transcription test
                return True
            else:
                print("   [FAIL] Recording failed - file empty or missing")
                return False
        except Exception as e:
    ...
    def test_whisper_transcription(self) -> bool:
        ...
        try:
            engine = getattr(self.config.whisper, 'engine', 'cli')
            print(f"   Engine: {engine}")
            print(f"   Model: {Path(self.config.whisper.model_path).name}")
            print("   [OK] Whisper initialized")
        except Exception as e:
            print(f"   [FAIL] Whisper initialization error: {e}")
            return False
        ...
        if not sample_audio:
            print("   [SKIP] No sample audio found - skipping transcription test")
        ...
        try:
            ...
            if cleaned.strip():
                ...
                self.results["performance"]["transcription_speed"] = len(cleaned)/runtime
                print("   [OK] Transcription successful")
                return True
            else:
                print("   [FAIL] Transcription produced empty result")
                return False
    ...
    def test_summarizer_service(self) -> bool:
        ...
        try:
            if self.summarizer.health_check():
                ...
                print("   [FAIL] Ollama service not responding")
        ...
        try:
            ...
            print(f"   Speed: {len(result.summary)/runtime:.1f} chars/sec")
            ...
            if result.summary.strip():
                ...
                self.results["performance"]["summarization_speed"] = len(result.summary)/runtime
                print("   [OK] Summarization successful")
                return True
            else:
                print("   [FAIL] Summarization produced empty result")
                return False
    ...
    def test_storage_system(self) -> bool:
        ...
        try:
            if self.storage.sessions_dir.exists():
                session_count = len(list(self.storage.sessions_dir.glob("session_*")))
                print(f"   [OK] Storage directory exists")
                print(f"   Found {session_count} session(s)")
            else:
                print("   [WARN] Storage directory does not exist (will be created)")
    ...
    def test_end_to_end(self) -> bool:
        ...
        try:
            ...
            input_devices = [d for d in devices if d.get("max_input_channels", 0) > 0]
            if not input_devices:
                print("   [SKIP] No audio input - skipping end-to-end test")
                return True
        except:
            print("   [SKIP] Audio system unavailable - skipping end-to-end test")
            return True
        
        print("\n1. Recording test audio (5 seconds)...")
        temp_dir = Path(self.config.storage.root) / "tmp"
        temp_dir.mkdir(parents=True, exist_ok=True)
        ...
        try:
            ...
            if not cleaned.strip():
                print("   [FAIL] Transcription produced no text")
                return False
            
            print(f"   [OK] Transcription complete ({runtime:.2f}s, {len(cleaned)} chars)")
        except Exception as e:
            print(f"   [FAIL] Transcription error: {e}")
            return False
        ...
        try:
            if not self.summarizer.health_check():
                print("   [SKIP] Ollama not running - skipping summarization")
            ...
            if not summary_result.summary.strip():
                print("   [FAIL] Summarization produced no text")
                return False
            
            print(f"   [OK] Summarization complete ({runtime:.2f}s, {len(summary_result.summary)} chars)")
            print(f"\n   Summary preview:")
    ...
    def test_performance(self) -> bool:
        ...
        if "transcription_runtime" in perf:
            ...
            trans_speed = perf.get("transcription_speed", 0)
            if trans_len > 0:
                print(f"\n   Transcription:")
                print(f"   Speed: {trans_speed:.1f} chars/sec")
                print(f"   Runtime: {trans_time:.2f}s")
                print(f"   Length: {trans_len} characters")
        
        if "summarization_runtime" in perf:
            summ_time = perf["summarization_runtime"]
            summ_len = perf.get("summarization_length", 0)
            summ_speed = perf.get("summarization_speed", 0)
            if summ_len > 0:
                print(f"\n   Summarization:")
                print(f"   Speed: {summ_speed:.1f} chars/sec")
                print(f"   Runtime: {summ_time:.2f}s")
                print(f"   Length: {summ_len} characters")
    ...
    def test_accuracy(self) -> bool:
        ...
        # Check if we have existing sessions with transcripts
        sessions_dir = self.storage.sessions_dir
        if not sessions_dir.exists():
            print("   [SKIP] No sessions found for accuracy analysis")
            return True
        
        sessions = list(sessions_dir.glob("session_*"))
        if not sessions:
            print("   [SKIP] No sessions found for accuracy analysis")
            return True
        ...
        for session_dir in sessions[:5]:  # Analyze first 5
            ...
            if transcript_path.exists():
                try:
                    ...
                    if summary_path.exists():
                        ...
                        compression_ratio = len(summary) / len(transcript) if transcript else 0
                        print(f"   Session {session_dir.name}:")
                        print(f"     Transcript: {len(transcript)} chars")
                        print(f"     Summary: {len(summary)} chars")
                        print(f"     Compression: {compression_ratio:.2%}")
                except Exception as e:
                    print(f"   [WARN] Error reading {session_dir.name}: {e}")
        
        if sessions_with_data > 0:
            avg_chars = total_chars / sessions_with_data
            print(f"\n   Average transcript length: {avg_chars:.0f} characters")
            print(f"   Sessions analyzed: {sessions_with_data}")
            print("   [OK] Accuracy analysis complete")
        else:
            print("   [SKIP] No transcript data found")
    ...
    def print_summary(self):
        ...
        if self.results["tests_failed"] == 0:
            ...
            print(f"[WARNING] {self.results['tests_failed']} TEST(S) FAILED")
        print("="*70 + "\n")

...
class DataTrainingArguments:
    ...
    dataset_name: Optional[str] = field(
        default=None,
        metadata={
            "help": "Dataset identifier for `load_dataset`. Leave None when supplying --train_file/--validation_file."
        },
    )
    train_file: Optional[str] = field(
        default=None,
        metadata={"help": "Local JSON/JSONL manifest that provides the training split when dataset_name is None."},
    )
    validation_file: Optional[str] = field(
        default=None,
        metadata={
            "help": "Local JSON/JSONL manifest that provides the evaluation split when dataset_name is None."
        },
    )
...
def main():
    ...
    if training_args.do_train:
        if data_args.dataset_name:
            ...
            raise ValueError("When do_train=True you must specify either --dataset_name or --train_file.")
    ...
    if training_args.do_eval:
        if data_args.dataset_name:
            ...
            raise ValueError("When do_eval=True you must specify either --dataset_name or --validation_file.")

    if not raw_datasets:
        raise ValueError("No dataset was loaded. Enable --do_train/--do_eval with matching data inputs.")

    sample_split = next(iter(raw_datasets.keys()))
    sample_dataset = raw_datasets[sample_split]
    dataset_source = (
        data_args.dataset_name
        if data_args.dataset_name
        else (data_args.train_file if sample_split == "train" else data_args.validation_file)
    )

    if data_args.audio_column_name not in sample_dataset.column_names:
        raise ValueError(
            f"--audio_column_name '{data_args.audio_column_name}' not found in dataset '{dataset_source}'. "
            "Make sure to set `--audio_column_name` to the correct audio column - one of "
            f"{', '.join(sample_dataset.column_names)}."
        )

    if data_args.text_column_name not in sample_dataset.column_names:
        raise ValueError(
            f"--text_column_name '{data_args.text_column_name}' not found in dataset '{dataset_source}'. "
    ...
    if model_args.freeze_feature_encoder:
        if hasattr(model, "freeze_feature_encoder"):
            ...
            logger.warning("--freeze_feature_encoder ignored: current model does not expose this helper.")
    ...
    if model_args.freeze_encoder:
        if hasattr(model, "freeze_encoder"):
            ...
            logger.warning("--freeze_encoder ignored: current model does not expose this helper.")
    ...
    def prepare_dataset(batch):
        ...
        if isinstance(sample, str):
            audio_array, sampling_rate = sf.read(sample)
            sample = {"array": audio_array, "sampling_rate": sampling_rate}
        elif isinstance(sample, dict) and "array" not in sample:
            audio_path = sample.get("path")
            if audio_path is not None:
                audio_array, sampling_rate = sf.read(audio_path)
                sample = {"array": audio_array, "sampling_rate": sampling_rate}

        audio_array = np.asarray(sample["array"])
        if audio_array.ndim > 1:
            audio_array = np.mean(audio_array, axis=1)
        if audio_array.dtype != np.float32:
            audio_array = audio_array.astype(np.float32)
        sample["array"] = audio_array
        sample_sampling_rate = sample.get("sampling_rate", target_sampling_rate)
        if sample_sampling_rate != target_sampling_rate:
            # Whisper expects 16kHz input, so resample any mismatched audio on the fly.
            sample["array"] = librosa.resample(
                sample["array"],
                orig_sr=sample_sampling_rate,
                target_sr=target_sampling_rate,
            ).astype(np.float32)
            sample_sampling_rate = target_sampling_rate
